...
def graf(inputf):
    ...
    wrs=map(float, wr_speed)
    L = lambda x: x/1024/1024
    wrs = map( L, wrs)
    plt.plot(wrs, 'b-x', linewidth= 1.5)
    ...
    plt.ylabel(u'rate, MBps')    # обозначение оси ординат

    #plt.show()

    # Сохраняем диаграмму в файл
    image_name = inputf.split('.')
    img = image_name[0] + '_' + image_name[-1].split('_')[-1] + '_write_speed.png'
