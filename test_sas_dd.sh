...
then
  ...
  bash ./test_dd/SAS/3sas-stripe.sh ${NUM}
  
  sleep 60
  echo "Start 4sas-stripe.sh with ${NUM} scans"
  bash ./test_dd/SAS/4sas-stripe.sh ${NUM}
  
  sleep 60
  echo "Start 5sas-raidz.sh with ${NUM} scans"
  bash ./test_dd/SAS/5sas-raidz.sh ${NUM}
  
  sleep 60
  echo "Start 6sas-raidz+stripe.sh with ${NUM} scans"
  bash ./test_dd/SAS/6sas-raidz+stripe.sh ${NUM}
  
  sleep 60
  echo "Start 6sas-mirror+stripe.sh with ${NUM} scans"
  bash ./test_dd/SAS/6sas-mirror+stripe.sh ${NUM}
else
